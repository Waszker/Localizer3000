...
public class FlowLayout extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		case MeasureSpec.AT_MOST:
			...
			realControlThickness = Math.min(contentThickness,
					controlMaxThickness);
			break;
		case MeasureSpec.EXACTLY:
			realControlLength = controlMaxLength;
			// With this line control will be exactly this size (an all child
			// elements will be narrower in case of lack of thickness)
			// realControlThickness = controlMaxThickness;
			realControlThickness = Math.max(contentThickness,
					controlMaxThickness);
			break;
		default:
			realControlLength = contentLength;
	...
	private void calculateLinesAndChildsPosition(List<LineDefinition> lines) {
		...
		for (LineDefinition line : lines) {
			...
			for (ViewContainer child : line.getViews()) {
				...
				prevChildThickness += child.getLength()
						+ child.getSpacingLength();
			}
		}
	}
			...
			if (this.config.getOrientation() == HORIZONTAL) {
				((LayoutParams) child.getView().getLayoutParams()).setPosition(
						this.getPaddingLeft() + line.getLineStartLength()
								+ child.getInlineStartLength(),
						this.getPaddingTop() + line.getLineStartThickness()
								+ child.getInlineStartThickness());
				child.getView().measure(
						MeasureSpec.makeMeasureSpec(child.getLength(),
								MeasureSpec.EXACTLY),
						MeasureSpec.makeMeasureSpec(child.getThickness(),
								MeasureSpec.EXACTLY));
			} else {
				((LayoutParams) child.getView().getLayoutParams()).setPosition(
	...
	private void applyGravityToLine(LineDefinition line) {
		...
		int excessLength = line.getLineLength()
				- (lastChild.getLength() + lastChild.getInlineStartLength());
		int excessOffset = 0;
		for (ViewContainer child : line.getViews()) {
			...
			final int childLength = child.getLength()
					+ child.getSpacingLength();
			final int childThickness = child.getThickness()
					+ child.getSpacingThickness();
			Rect container = new Rect();
			container.top = 0;
	...
	private void drawDebugInfo(Canvas canvas, View child) {
		...
		if (lp.horizontalSpacing > 0) {
			...
			float x = child.getRight();
			float y = child.getTop() + child.getHeight() / 2.0f;
			canvas.drawLine(x, y, x + this.config.getHorizontalSpacing(), y,
					layoutPaint);
			canvas.drawLine(x + this.config.getHorizontalSpacing() - 4.0f,
					y - 4.0f, x + this.config.getHorizontalSpacing(), y,
					layoutPaint);
			canvas.drawLine(x + this.config.getHorizontalSpacing() - 4.0f,
					y + 4.0f, x + this.config.getHorizontalSpacing(), y,
					layoutPaint);
		...
		if (lp.verticalSpacing > 0) {
			...
			float x = child.getLeft() + child.getWidth() / 2.0f;
			float y = child.getBottom();
			canvas.drawLine(x, y, x, y + this.config.getVerticalSpacing(),
					layoutPaint);
			canvas.drawLine(x - 4.0f, y + this.config.getVerticalSpacing()
					- 4.0f, x, y + this.config.getVerticalSpacing(),
					layoutPaint);
			canvas.drawLine(x + 4.0f, y + this.config.getVerticalSpacing()
					- 4.0f, x, y + this.config.getVerticalSpacing(),
					layoutPaint);
